	...
	public static void addRecipe() {
		for(Material m : Material.MATERIAL_REGISTRY) {
			if(m.hasFlag(DustMaterial.MatFlags.GENERATE_ORE)) {
				...
				RecipeOutput[] oreRecipeOutputs = {
												   new RecipeOutput(OreDictUnifier.get(OrePrefix.crushed, m), 0.5F, 0.8F), 
												   new RecipeOutput(OreDictUnifier.get(OrePrefix.crushed, byproductMaterial), 1.4F, 7.2F)
												  };							
				...
				if(GTUtility.selectItemInList(1, ((DustMaterial)m), ((DustMaterial)m).oreByProducts, DustMaterial.class) != null) {
					...
					RecipeOutput[] crushedRecipeOutputs = {
							   							   new RecipeOutput(OreDictUnifier.get(OrePrefix.dustImpure, m), 0.5F, 0.8F), 
							   							   new RecipeOutput(OreDictUnifier.get(OrePrefix.dustImpure, byproductMaterial), 1.1F, 4.2F),
							   							   new RecipeOutput(OreDictUnifier.get(OrePrefix.dustImpure, crushedTertiaryproductMaterial), 1.2F, 9.2F)
							  							  };							
					...
					RecipeOutput[] crushedRecipeOutputs = {
							   							   new RecipeOutput(OreDictUnifier.get(OrePrefix.dustImpure, m), 0.5F, 0.8F), 
							   							   new RecipeOutput(OreDictUnifier.get(OrePrefix.dustImpure, byproductMaterial), 1.4F, 7.2F)
							  							  };							
					SAGMillRecipe crushedRecipe = new SAGMillRecipe(crushedRecipeInput, 13500, RecipeBonusType.CHANCE_ONLY, crushedRecipeOutputs);
					SagMillRecipeManager.getInstance().addRecipe(crushedRecipe);
				}
