...
public class TraitDiggersLuck extends Trait {
    ...
    /**
     * - Skill level affects what drops you will get - Fortunate affects how many
     * drops you will get
     */
    @Override
    public void onBlockDrops(HarvestDropsEvent event) {
        ...
        int amount = 1 + event.getFortuneLevel(), skillLevel = PlayerDataHandler.get(player).getSkillInfo(getParentSkill()).getLevel();

        /**
         * Accessible at 5, but uncommon is only gotten after 15, rare = 25, epic = max.
         * Not sure if I should use isToolEffective or getMaterial to determine... Also,
         * since we have no way to find out if mined blocks are natural or artificial we
         * have to remove all drops (unless the roundabout way MCMMO does it where its
         * sorta taxing)...
         */
        if (block.isToolEffective("shovel", state) && (player.getHeldItemMainhand().getItem() instanceof ItemSpade/* || player.getHeldItemMainhand().getItem() instanceof Shovel*/)) {
            /*
